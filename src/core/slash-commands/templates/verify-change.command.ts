...
export function getVerifyChangeSlashCommand(): SlashCommandTemplate {
  return {
    name: "DesignSpec: Verify",
    ...
    instructions: dedent`
      ...
      **Input**: Optionally specify a change name after \`/desx:verify\` (e.g., \`/desx:verify add-space-management\`). If omitted, check if it can be inferred from conversation context. If vague or ambiguous you MUST prompt for available changes.
