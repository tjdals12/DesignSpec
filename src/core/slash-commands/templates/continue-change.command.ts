...
export function getContinueChangeSlashCommand(): SlashCommandTemplate {
  return {
    name: "DesignSpec: Continue",
    ...
    instructions: dedent`
      ...
      **Input**: Optionally specify a change name after \`/desx:continue\` (e.g., \`/desx:continue add-space-management\`). If omitted, check if it can be inferred from conversation context. If vague or ambiguous you MUST prompt for available changes.

      **Steps**

      1. **If no change name provided, prompt for selection**

         Run \`design-spec list --json\` to get available changes sorted by most recently modified. Then use the **AskUserQuestion tool** to let the user select which change to work on.
      ...
      3. **Act based on status**:
         ...
         **If all artifacts are complete (\`isComplete: true\`)**:
         ...
         - Suggest: "All artifacts created! You can now implement this change with \`/desx:apply\` or archive it with \`/desx:archive\`."
         - STOP
      ...
      **Output**
      ...
      - Prompt: "Run \`/desx:continue\` to create the next artifact"
