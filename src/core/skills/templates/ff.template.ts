...
export function getFastForwardSkillTemplate(): SkillTemplate {
  return {
    ...
    **Input**: The user's request should include a change name (kebab-case) OR a description of what they want to design or build.
    ...
    **Steps**
    ...
    4. **Loop: create artifacts until every \`applyRequires\` artifact is \`"done"\`**
       ...
       **If an artifact requires user input** (scope, naming, or design choice that materially shapes the artifact):
       ...
       - Prefer making a reasonable decision and noting it inline (in the artifact) over stalling on every minor uncertainty — this skill's value is momentum.
    ...
    **Output**
    ...
    - Prompt: "All artifacts created — ready for implementation. Ask me to apply the change to start working on the tasks."
