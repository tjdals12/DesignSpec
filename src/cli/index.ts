...
program
  ...
  .option("--json", "Output as JSON")
  .action(async (targetPath = ".", options: { change: string; json: boolean }) => {
    try {
      const resolvedPath = path.resolve(targetPath);
      ...
      const { StatusCommand } = await import("#workflows/commands/status.js");
...
program
  ...
  .action(async (targetPath: string = ".", options: { change: string }) => {
    try {
      ...
      const { NewChangeCommand } = await import("#workflows/commands/new-change.js");
